...
function onMobDespawn(mob)
    ...
    if LandKingSystem_HQ ~= 1 and ToD <= os.time() and popNow then
        ...
        GetMobByID(ID.mob.ASPIDOCHELONE):setRespawnTime(30) -- (75600 + math.random(0, 6) * 1800) -- 21 - 24 hours with half hour windows
    else
        if LandKingSystem_NQ ~= 1 then
            UpdateNMSpawnPoint(ID.mob.ADAMANTOISE)
            GetMobByID(ID.mob.ADAMANTOISE):setRespawnTime(30) -- (75600 + math.random(0, 6) * 1800) -- 21 - 24 hours with half hour windows
